...
Phone: (501) 464-5139
Email: office@smiroof.com (new primary public email — set June 2026, replaced info@)
Website: smiroof.com
...
Financing: Wisetack
Email: office@smiroof.com — primary public-facing email on all 160 pages (visible footer link + JSON-LD schema) as of June 2026; replaced info@smiroof.com sitewide. smiroofingcompany@gmail.com still active (internal/Git). info@smiroof.com retired from the site. cory@smiroof.com not confirmed active.
...
- DECISION FOR CORY (June 2026 email migration): the public site email is now office@smiroof.com everywhere, but the INTERNAL lead-notification recipient in /api/contact.js is intentionally still the `dmcorysmith@gmail.com` fallback — NOT rerouted to office@, to avoid leads landing in an unverified mailbox. If Cory wants leads to go to the new shared inbox, set `LEAD_NOTIFY_TO=office@smiroof.com` in Vercel (no code change needed). Also confirm office@smiroof.com is actively receiving mail before relying on it.
...
- Confirm office@smiroof.com is actively receiving — it is now the public contact email on all 160 pages. cory@smiroof.com not confirmed active.
...
| 2026-06-05 | Claude | service-areas/{russellville,pottsville,dardanelle,dover,london}-ar/ + AGENTS.md + CLAUDE.md | Session 10 — residential local-anchor enrichment (Cory asked to put the real landmarks/employers on the RESIDENTIAL pages too, not just commercial — earlier only Russellville had gotten Nuclear One/ATU). Wove verified local anchors into all 5 home-base residential coverage ledes: Russellville +Saint Mary's Regional Medical Center +Lake Dardanelle State Park +ATU campus (already had Nuclear One); Pottsville +Pottsville School District + U.S. 64 growth; Dardanelle +Mount Nebo State Park + "one of Yell County's two county seats" + Lake Dardanelle waterfront; Dover +Dover School District +Long Pool on the Big Piney; London +Arkansas Nuclear One (London is ANO's host community). ALL claims adversarially fact-checked against sources — caught + fixed 3 before deploy: London's ANO was placed "east of town" (BACKWARDS — ANO is AT London, west of Russellville); dropped an unverifiable "fastest-growing district in Pope County" superlative on Pottsville; corrected "the Yell County seat" → "one of Yell County's two county seats" (Danville is the other). Verified: 0 broken links, all schema/faq/paa validators pass. STILL PENDING (needs Cory): real SMI Cam project photos for these 5 towns; GA4 + CallRail. |
| 2026-06-05 | Claude | NEW: service-areas/danville-ar/, commercial-roofing/danville-ar/ + MODIFIED service-areas/{clarksville,morrilton}-ar/, index.html, areas-we-serve, commercial-roofing/index.html, sitemap.xml, site-map + AGENTS.md + CLAUDE.md | Session 10 — River Valley core expansion (Cory: make Russellville, Dardanelle, Pottsville, Dover, London, Clarksville, Morrilton, Danville strong for res + commercial + repairs). Built DANVILLE (Yell County's western co-seat, ~24 mi SW of Russellville) — the one town in the set with NO page at all — as a new residential page (cloned from dardanelle-ar residential; PROTECTED the in-page service-area map list during the city-name swap so "Lake Dardanelle" wouldn't become "Lake Danville"; localized coverage lede + promise sentence + nearby towns with verified facts) AND a new commercial page (cloned from dardanelle-ar commercial; injected verified intro/why-choose/FAQs). Content came from a web-research → adversarial-fact-check workflow PLUS a final fact-check on the built pages, which caught a SYSTEMIC bug: the global Dardanelle→Danville swap had corrupted the intentional references to neighbor/other-seat Dardanelle (e.g. "sharing that role with Danville", a self-referential nearby pill, "plus nearby Danville and Russellville") — all fixed; also fixed Mount Magazine NW-not-S and Highway 27 direction. Enriched Clarksville (University of the Ozarks, Spadra Creek, Lamar/Coal Hill/Hartman/Knoxville) + Morrilton (Petit Jean State Park "Arkansas's first", UACCM, Petit Jean Meats, Green Bay Packaging) residential ledes with richer verified versions. Linked Danville from the homepage city list + areas-we-serve + commercial hub grid; added R+C to sitemap; ran sync scripts; regenerated site-map. Verified: 158→160 HTML, 158 sitemap URLs, 42 residential + 30 commercial city pages, 0 broken links, all schema/faq/paa validators pass. The other 5 towns were already strong from prior rounds. PENDING: add Danville to the other 41 pages' coverage-list map (cosmetic internal-link gap); real SMI Cam photos; GA4 + CallRail. |
| 2026-06-19 | Claude | 160 HTML + 5 generator/sync scripts + AGENTS.md + CLAUDE.md | Session 11 — new business email migration (Cory: "update the email everywhere, especially commercial"). Replaced info@smiroof.com → office@smiroof.com sitewide — 339 display/mailto + JSON-LD-schema occurrences across 118 pages — PLUS the hardcoded email constant in 5 generator/sync scripts (sync-city-roofingcontractor-schema, generate-comparison-pages, generate-state-farm-claim-pages, generate-storm-event-pages, generate-html-sitemap) so the old address can never be regenerated. Closed the coverage gap: ALL commercial/industries pages already carried the email (visible footer + schema); the 42 residential service-areas/*-ar pages had NONE — added a visible footer mailto link AND an `email` field (placed right after `telephone`) in their RoofingContractor JSON-LD. Also added a visible footer email to the 16 remaining schema-only pages (homepage, site-map, reviews, video-center, commercial-roofing-cost + the cost/comparison pages). Result: 160/160 pages now carry BOTH a visible office@ link and the schema email. Executed via 2 asserted /tmp codemods (the global replace proved its only textual delta was the email string, so the A2P block, nav, footer, and schema structure stayed byte-identical; the schema additions were re-parsed as JSON per page). Verified: 0 info@ left anywhere (html/js/mjs), 646 JSON-LD blocks all parse, A2P comment + cf_sms_consent intact, predeploy/schema-release/service/breadcrumb/blogposting/review/faq-library/city-schema all pass. Did NOT touch /api/contact.js — the INTERNAL lead-notification recipient is still the `dmcorysmith@gmail.com` fallback (protected backend file + office@ delivery unconfirmed); flagged for Cory to set `LEAD_NOTIFY_TO=office@smiroof.com` in Vercel if he wants leads in the new mailbox. paa-refresh "stale" warning is pre-existing (date-based, 18 days) and unrelated; did NOT mass-bump sitemap lastmod (cosmetic change). |

---

## LAST UPDATED

June 19, 2026 (session 11) — Claude
