...
corrected fallback address once SMTP is live. Note: the June 2026 site-wide
email migration changed the PUBLIC contact email to `office@smiroof.com`, but
this INTERNAL lead recipient was deliberately left on the existing fallback —
set `LEAD_NOTIFY_TO=office@smiroof.com` in Vercel to route incoming leads to the
new shared inbox.
...
| 2026-06-05 | Claude | 8 commercial service pages + 26 residential city pages + sitemap.xml + AGENTS.md + CLAUDE.md | Commercial + residential depth (session 10). Commercial: interlinked the 2 new commercial pages (inspections, emergency) into all 8 commercial service-page sidebars (each new page 1→12 inbound links); the 4 /industries/ pages were already substantive (~1,300 words + FAQPage), no expansion needed. Residential: localized the 26 city pages still on generic meta-boilerplate, replacing it with web-research-VERIFIED local content (county, region, nearby towns, highways, verified landmarks) produced by a research→adversarial-fact-check workflow that caught and corrected real factual errors (e.g. a wrongly-named highway, overstated distances); each localized lede links to the residential money pages; no fabrication. Bumped sitemap lastmod on the 26. Verified: 0 broken links, all schema/FAQ/PAA validators pass; coverage boilerplate 26→0. |
| 2026-06-05 | Claude | +2 commercial pages (dover-ar, london-ar) + service-areas/{russellville,dover,london}-ar + commercial-roofing/index.html + sitemap.xml + site-map + AGENTS.md + CLAUDE.md | Home-base depth (session 10). Built the 2 missing commercial city pages (Dover, London — both had residential but no commercial), cloned from dardanelle-ar with verified Pope County content and full Service/FAQPage/Breadcrumb schema; linked from the commercial hub + cross-linked the residential pages; enriched Russellville residential with Arkansas Nuclear One + ATU. All new local claims adversarially fact-checked against sources — fixed 3 errors before deploy (Dover former-vs-original county seat, London distance ~12→~9 mi, removed a non-existent London School District). Left Pottsville commercial alone (already well-localized). Verified: 158 HTML, 156 sitemap URLs, 0 broken links, all validators pass. |
| 2026-06-05 | Claude | 5 residential city pages + AGENTS.md + CLAUDE.md | Residential local-anchor enrichment (session 10). Added verified local landmarks/employers to all 5 home-base residential coverage ledes (Russellville: Saint Mary's + Lake Dardanelle State Park + ATU; Pottsville: school district + growth; Dardanelle: Mount Nebo State Park + two-county-seats; Dover: Dover School District + Long Pool; London: Arkansas Nuclear One host community). Adversarially fact-checked; fixed 3 errors pre-deploy (ANO direction backwards, an unverifiable superlative, the two-county-seats precision). 0 broken links, all validators pass. |
| 2026-06-05 | Claude | +2 Danville pages (service-areas/danville-ar, commercial-roofing/danville-ar) + 2 enriched ledes (clarksville, morrilton) + homepage/areas-we-serve/commercial-hub links + sitemap + site-map + AGENTS.md + CLAUDE.md | River Valley core expansion (session 10). Built Danville residential + commercial (Yell County co-seat; the one River Valley core town with no page) by cloning the Dardanelle pages and localizing with web-verified facts; enriched Clarksville + Morrilton residential ledes with richer verified content. All content adversarially fact-checked — a final pass on the built pages caught a systematic global-swap bug that had turned intentional "Dardanelle" neighbor references into "Danville" self-references, plus direction errors; all fixed. Linked Danville sitewide (homepage, areas-we-serve, commercial hub, sitemap). Verified: 160 HTML, 158 sitemap URLs, 42 res + 30 com cities, 0 broken links, all validators pass. |
| 2026-06-19 | Claude | 160 HTML + 5 generator/sync scripts + AGENTS.md + CLAUDE.md | New business email migration (session 11). Replaced info@smiroof.com → office@smiroof.com across the whole site — 339 display/mailto + JSON-LD-schema occurrences on 118 pages — and updated the hardcoded email constant in 5 generator/sync scripts (sync-city-roofingcontractor-schema, generate-comparison-pages, generate-state-farm-claim-pages, generate-storm-event-pages, generate-html-sitemap) so regeneration can't reintroduce the old address. Filled the coverage gap: every commercial/industries page already had the email; the 42 residential service-areas/*-ar pages had NONE — added a visible footer mailto link + an `email` field (after `telephone`) to their RoofingContractor JSON-LD; also added a visible footer email to the 16 remaining schema-only pages (homepage, site-map, reviews, video-center, commercial-roofing-cost + the cost/comparison pages). Result: 160/160 pages now carry both a visible office@ link and the schema email. Executed via asserted /tmp codemods (the global replace's only textual delta was the email string, so the A2P block, nav, footer, and schema structure stayed byte-identical; schema additions re-parsed as JSON per page). Verified: 0 info@ remaining (html/js/mjs), 646 JSON-LD blocks all parse, A2P comment + cf_sms_consent intact, predeploy/schema-release/service/breadcrumb/blogposting/review/faq-library/city-schema all pass. /api/contact.js NOT changed — the internal lead-notification recipient stays the dmcorysmith@gmail.com fallback (protected backend file + office@ delivery unconfirmed); set `LEAD_NOTIFY_TO=office@smiroof.com` in Vercel to reroute leads. paa-refresh "stale" warning is pre-existing (date-based) and unrelated. |
